...
TEST(main, DeviceAllocator) {
  ...
  auto p1 = memory.alloc(64,1,0,0,false);
  memory.free(p1);
  }
...
TEST(main, DeviceAllocator2) {
  ...
  auto p1 = memory.alloc(64, 1,0,0, false);
  auto p2 = memory.alloc(128,1,0,0, false);
  auto p3 = memory.alloc(32, 1,0,0, false);
  memory.free(p1);
  memory.free(p2);
  memory.free(p3);
  }
...
TEST(main, DeviceAllocatorMergeBlock) {
  ...
  auto p1 = memory.alloc(64, 1,0,0, false);
  auto p2 = memory.alloc(64, 1,0,0, false);
  auto p3 = memory.alloc(64, 1,0,0, false);
  auto p4 = memory.alloc(64, 1,0,0, false);
  auto p5 = memory.alloc(64, 1,0,0, false);
  memory.free(p4);
  memory.free(p2);
  memory.free(p3);
  memory.free(p1);
...
TEST(main, DeviceAllocatorAlign0) {
  ...
  auto p1 = memory.alloc(64,  4,0,0, false);
  auto p2 = memory.alloc(big, 5,0,0, false);
  auto p3 = memory.alloc(32,  6,0,0, false);
  memory.free(p2);
  memory.free(p1);
  memory.free(p3);
  }
