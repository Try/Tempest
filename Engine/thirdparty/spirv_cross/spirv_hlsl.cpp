...
void CompilerHLSL::emit_interface_block_in_struct(const SPIRVariable &var, unordered_set<uint32_t> &active_locations)
{
	...
	if (use_location_number)
	{
		...
		if (is_tessellation_shader() && !type.array.empty())
		{
...
void CompilerHLSL::emit_builtin_variables()
{
	...
	builtins.for_each_bit([&](uint32_t i) {
		...
		case BuiltInTessCoord:
			...
			break;
		case BuiltInTessLevelOuter:
			array_size = get_entry_point().flags.get(ExecutionModeTriangles) ? 3 : 4;
			type = "float";
			break;
		case BuiltInTessLevelInner:
			array_size = get_entry_point().flags.get(ExecutionModeTriangles) ? 0 : 2;
			type = "float";
			break;
...
void CompilerHLSL::emit_resources()
{
	...
	ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable &var) {
		auto &type = this->get<SPIRType>(var.basetype);
		bool is_cp_variable = (execution.model == ExecutionModelTessellationEvaluation && var.storage == StorageClassInput && !has_decoration(var.self,DecorationPatch));
...
void CompilerHLSL::extract_global_variables_from_functions(VariableID gl_in)
{
	if (gl_in==0)
		return;
	unordered_set<uint32_t> processed_func_ids;
	extract_global_variables_from_function(gl_in, ir.default_entry_point, processed_func_ids);
}

void CompilerHLSL::extract_global_variables_from_function(VariableID gl_in, uint32_t func_id,
                                                          unordered_set<uint32_t> &processed_func_ids)
{
			...
			switch (op)
			{
				case OpFunctionCall:
				{
				  ...
				  uint32_t inner_func_id = ops[2];
					extract_global_variables_from_function(gl_in, inner_func_id, processed_func_ids);
					break;
				}
				default:
					break;
...
void CompilerHLSL::emit_hlsl_entry_point()
{
	...
	case ExecutionModelTessellationControl:
		...
		if (execution.flags.get(ExecutionModePointMode))
			statement("[outputtopology(\"point\")]");
...
void CompilerHLSL::emit_per_vertex_inputs()
{
	...
	extract_global_variables_from_functions(gl_in.self);
}
